    ...
    public void execute(){
        ...
        while(true) {
            try{
                ...
                LOGGER.error("Ошибка работы БД", e);
            }
        }
    }

    public void doAccept(SelectionKey key) throws IOException, SQLException {
        ServerSocketChannel ssc = (ServerSocketChannel) key.channel();
        SocketChannel sc = ssc.accept();
        LOGGER.info("Клиент подключился: " + sc.getLocalAddress());
        LOGGER.info("Новое подключение клиента: {}", sc.getRemoteAddress());
        sc.configureBlocking(false);
        SelectionKey clientKey = sc.register(key.selector(), OP_READ, new Attach(0));

        Attach attach = (Attach) clientKey.attachment();
        Communicator comm = new Communicator();
        attach.setCommunicator(comm);

        sendingResponse((Serializable) attach.getCommunicator().getUsagesCommands(), clientKey);
